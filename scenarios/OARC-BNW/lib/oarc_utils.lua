...
-- Useful constants
--------------------------------------------------------------------------------
STARTING_SPAWN_CHUNK_SIZE = 32		-- added to reduce size of initial spawn area
CHUNK_SIZE = 32		-- vf - use config.lua to set circle size - leave this 32 - which is the game chunk size
MAX_FORCES = 64
TICKS_PER_SECOND = 60
...
-- Gets chunk position of a tile.  changed all 32's in CHUNK_SIZE here  vf
function GetChunkPosFromTilePos(tile_pos)
    return {x=math.floor(tile_pos.x/CHUNK_SIZE), y=math.floor(tile_pos.y/CHUNK_SIZE)}
end

function GetCenterTilePosFromChunkPos(c_pos)
    return {x=c_pos.x*CHUNK_SIZE + (CHUNK_SIZE/2), y=c_pos.y*CHUNK_SIZE + (CHUNK_SIZE/2)}
end

-- Get the left_top
function GetChunkTopLeft(pos)
    return {x=pos.x-(pos.x % CHUNK_SIZE), y=pos.y-(pos.y % CHUNK_SIZE)}
end
...
function CoverAreaInTiles(surface, area, tile_name)
    tiles = {}
    for x = area.left_top.x,area.left_top.x+(CHUNK_SIZE-1) do
        for y = area.left_top.y,area.left_top.y+(CHUNK_SIZE-1) do
...
function CreateHoldingPen(surface, chunkArea)
    local radiusTiles = global.ocfg.spawn_config.gen_settings.land_area_tiles-37
    if (((chunkArea.left_top.x >= -(radiusTiles+2*CHUNK_SIZE)) and (chunkArea.left_top.x <= (radiusTiles+2*CHUNK_SIZE))) and
        ...
        CreateMoat(surface, {x=0,y=0}, chunkArea, radiusTiles+14, "out-of-map", false)
        CreateMoat(surface, {x=0,y=0}, chunkArea, 4, "out-of-map", false)
    end
end
